...
class map_ResourceMap {
    ...
    constructor(arg, toKey) {
        ...
        if (arg instanceof map_ResourceMap) {
            this.map = ( new Map(arg.map));
            this.toKey = toKey ?? map_ResourceMap.defaultToKey;
        }
            ...
            this.map = ( new Map());
            this.toKey = arg ?? map_ResourceMap.defaultToKey;
        }
    }
    set(resource, value) {
...
class ResourceSet {
    constructor(entriesOrKey, toKey) {
        ...
        if (!entriesOrKey || typeof entriesOrKey === 'function') {
            this._map = ( new map_ResourceMap(entriesOrKey));
        }
        else {
            this._map = ( new map_ResourceMap(toKey));
...
const platform_isLinux = _isLinux;
...
function platform_isLittleEndian() {
...
function strings_escapeRegExpCharacters(value) {
...
function trim(haystack, needle = ' ') {
    const trimmed = strings_ltrim(haystack, needle);
...
function createRegExp(searchString, isRegex, options = {}) {
    ...
    if (!isRegex) {
        searchString = strings_escapeRegExpCharacters(searchString);
    }
...
function strings_isHighSurrogate(charCode) {
...
function getNextCodePoint(str, len, offset) {
    ...
    if (strings_isHighSurrogate(charCode) && offset + 1 < len) {
...
function getPrevCodePoint(str, offset) {
    ...
    if (isLowSurrogate(charCode) && offset > 1) {
        ...
        if (strings_isHighSurrogate(prevCharCode)) {
...
function startsWithUTF8BOM(str) {
    return !!(str && str.length > 0 && str.charCodeAt(0) === charCode_CharCode.UTF8_BOM);
}
...
class StringSHA1 {
    ...
    update(str) {
        ...
        while (true) {
            ...
            if (strings_isHighSurrogate(charCode)) {
                if (offset + 1 < strLen) {
                    const nextCharCode = str.charCodeAt(offset + 1);
                    if (isLowSurrogate(nextCharCode)) {
                        offset++;
...
const path_extname = (platformIsWin32 ? win32.extname : path_posix.extname);
const parse = (platformIsWin32 ? win32.parse : path_posix.parse);
const path_sep = (platformIsWin32 ? win32.sep : path_posix.sep);
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/editor/common/model/pieceTreeTextBuffer/rbTreeBase.js
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/editor/common/model/pieceTreeTextBuffer/pieceTreeBase.js
...
function createLineStartsFast(str, readonly = true) {
    ...
    for (let i = 0, len = str.length; i < len; i++) {
        ...
        if (chr === charCode_CharCode.CarriageReturn) {
            if (i + 1 < len && str.charCodeAt(i + 1) === charCode_CharCode.LineFeed) {
                r[rLength++] = i + 2;
                i++;
            }
            else {
...
function createLineStarts(r, str) {
    ...
    for (let i = 0, len = str.length; i < len; i++) {
        ...
        if (chr === charCode_CharCode.CarriageReturn) {
            ...
            if (isBasicASCII) {
                if (chr !== charCode_CharCode.Tab && (chr < 32 || chr > 126)) {
                    isBasicASCII = false;
                }
            }
        }
...
class PieceTreeBase {
        ...
        while (x !== SENTINEL) {
            if (x.size_left !== 0 && x.size_left >= offset) {
                ...
                if (out.index === 0) {
                    ...
                    return ( new position_Position(lfCnt + 1, column + 1));
                }
                return ( new position_Position(lfCnt + 1, out.remainder + 1));
            }
            else {
    ...
    getLinesContent() {
        ...
        this.iterate(this.root, node => {
            ...
            if (danglingCR) {
                if (buffer.charCodeAt(pieceStartOffset) === charCode_CharCode.LineFeed) {
                    pieceStartOffset++;
                    pieceLength--;
                }
            ...
            if (pieceStartLine === pieceEndLine) {
                if (!this._EOLNormalized && buffer.charCodeAt(pieceStartOffset + pieceLength - 1) === charCode_CharCode.CarriageReturn) {
                    danglingCR = true;
            ...
            if (!this._EOLNormalized && buffer.charCodeAt(lineStarts[pieceEndLine] + piece.end.column - 1) === charCode_CharCode.CarriageReturn) {
                danglingCR = true;
    ...
    findMatchesInNode(node, searcher, startLineNumber, startColumn, startCursor, endCursor, searchData, captureMatches, limitResultCount, resultLen, result) {
        ...
        do {
            ...
            if (m) {
                ...
                result[resultLen++] = createFindMatch(( new range_Range(
    ...
    _findMatchesInLine(searchData, searcher, text, lineNumber, deltaOffset, resultLen, result, captureMatches, limitResultCount) {
        ...
        if (!captureMatches && searchData.simpleSearch) {
            ...
            while ((lastMatchIndex = text.indexOf(searchString, lastMatchIndex + searchStringLen)) !== -1) {
                if (!wordSeparators || isValidMatch(wordSeparators, text, textLength, lastMatchIndex, searchStringLen)) {
                    result[resultLen++] = ( new model_FindMatch(( new range_Range(
                        lineNumber,
                        lastMatchIndex + 1 + deltaOffset,
                        lineNumber,
                        lastMatchIndex + 1 + searchStringLen + deltaOffset
    ...
    createNewPieces(text) {
        if (text.length > AverageBufferSize) {
            ...
            while (text.length > AverageBufferSize) {
                ...
                let splitText;
                if (lastChar === charCode_CharCode.CarriageReturn || (lastChar >= 0xD800 && lastChar <= 0xDBFF)) {
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/editor/common/core/stringBuilder.js
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/editor/common/core/textChange.js
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/editor/common/model/pieceTreeTextBuffer/pieceTreeTextBuffer.js
...
class PieceTreeTextBuffer extends lifecycle_Disposable {
    ...
    getValueInRange(range, eol = model_EndOfLinePreference.TextDefined) {
        if (range.isEmpty()) {
            return '';
        }
    ...
    getValueLengthInRange(range, eol = model_EndOfLinePreference.TextDefined) {
        if (range.isEmpty()) {
            return 0;
        }
    ...
    getCharacterCountInRange(range, eol = model_EndOfLinePreference.TextDefined) {
        if (this._mightContainNonBasicASCII) {
            ...
            for (let lineNumber = fromLineNumber; lineNumber <= toLineNumber; lineNumber++) {
                ...
                for (let offset = fromOffset; offset < toOffset; offset++) {
                    if (strings_isHighSurrogate(lineContent.charCodeAt(offset))) {
                        result = result + 1;
                        offset = offset + 1;
                    }
                    else {
    ...
    _getEndOfLine(eol) {
        switch (eol) {
            case model_EndOfLinePreference.LF:
                return '\n';
            case model_EndOfLinePreference.CRLF:
                return '\r\n';
            case model_EndOfLinePreference.TextDefined:
                return this.getEOL();
            default:
    ...
    _toSingleEditOperation(operations) {
        ...
        const entireEditRange = ( new range_Range(
        ...
        for (let i = 0, len = operations.length; i < len; i++) {
            ...
            result.push(this.getValueInRange(( new range_Range(lastEndLineNumber, lastEndColumn, range.startLineNumber, range.startColumn))));
        ...
        return {
            ...
            rangeLength: this.getValueLengthInRange(entireEditRange, model_EndOfLinePreference.TextDefined),
            text: text,
            eolCount: eolCount,
            firstLineLength: firstLineLength,
            lastLineLength: lastLineLength,
    ...
    _doApplyEdits(operations) {
        ...
        for (let i = 0; i < operations.length; i++) {
            ...
            const contentChangeRange = ( new range_Range(startLineNumber, startColumn, endLineNumber, endColumn));
            contentChanges.push({
    ...
    static _getInverseEditRange(range, text) {
        ...
        if (text.length > 0) {
            ...
            resultRange = ( new range_Range(startLineNumber, startColumn, startLineNumber, startColumn));
        }
        return resultRange;
    }
    ...
    static _sortOpsAscending(a, b) {
        const r = range_Range.compareRangesUsingEnds(a.range, b.range);
        if (r === 0) {
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/editor/common/model/pieceTreeTextBuffer/pieceTreeTextBufferBuilder.js
...
class PieceTreeTextBufferBuilder {
    ...
    _finish() {
        ...
        if (this._hasPreviousChar) {
            ...
            if (this._previousChar === charCode_CharCode.CarriageReturn) {
                this.cr++;
            }
        }
    }
...
function extpath_isEqualOrParent(base, parentCandidate, ignoreCase, separator = path_sep) {
...
class RemoteAuthoritiesImpl {
    ...
    get _remoteResourcesPath() {
        return path_posix.join(this._serverRootPath, network_Schemas.vscodeRemoteResource);
    }
...
class FileAccessImpl {
    constructor() {
        this.staticBrowserUris = ( new map_ResourceMap());
    ...
    uriToBrowserUri(uri) {
        if (uri.scheme === network_Schemas.vscodeRemote) {
            return RemoteAuthorities.rewrite(uri);
        }
        if (
        uri.scheme === network_Schemas.file &&
            (
            (
            isNative || (webWorkerOrigin === `${network_Schemas.vscodeFileResource}://${FileAccessImpl.FALLBACK_AUTHORITY}`)))) {
            return uri.with({
                scheme: network_Schemas.vscodeFileResource,
    ...
    uriToFileUri(uri) {
        if (uri.scheme === network_Schemas.vscodeFileResource) {
            return uri.with({
                scheme: network_Schemas.file,
...
class ExtUri {
    ...
    isEqualOrParent(base, parentCandidate, ignoreFragment = false) {
        if (base.scheme === parentCandidate.scheme) {
            if (base.scheme === network_Schemas.file) {
                return extpath_isEqualOrParent(originalFSPath(base), originalFSPath(parentCandidate), this._ignorePathCasing(base)) && base.query === parentCandidate.query && (ignoreFragment || base.fragment === parentCandidate.fragment);
            }
            if (isEqualAuthority(base.authority, parentCandidate.authority)) {
                return extpath_isEqualOrParent(base.path, parentCandidate.path, this._ignorePathCasing(base), '/') && base.query === parentCandidate.query && (ignoreFragment || base.fragment === parentCandidate.fragment);
            }
        }
        return false;
    }
    ...
    dirname(resource) {
        ...
        let dirname;
        if (resource.scheme === network_Schemas.file) {
    ...
    relativePath(from, to) {
        ...
        if (from.scheme === network_Schemas.file) {
    ...
    hasTrailingPathSeparator(resource, sep$1 = path_sep) {
        if (resource.scheme === network_Schemas.file) {
    ...
    removeTrailingPathSeparator(resource, sep$1 = path_sep) {
        if (hasTrailingPathSeparator(resource, sep$1)) {
            return resource.with({ path: resource.path.substr(0, resource.path.length - 1) });
        }
        return resource;
    }
    addTrailingPathSeparator(resource, sep$1 = path_sep) {
        let isRootSep = false;
        if (resource.scheme === network_Schemas.file) {
            const fsp = originalFSPath(resource);
...
const extUriBiasedIgnorePathCase = ( new ExtUri(uri => {
    return uri.scheme === network_Schemas.file ? !platform_isLinux : true;
}));
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/base/common/glob.js
...
function glob_parse(arg1, options = {}) {
    ...
    if (typeof arg1 === 'string' || glob_isRelativePattern(arg1)) {
...
function glob_isRelativePattern(obj) {
    const rp = obj;
    if (!rp) {
        return false;
    }
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/base/common/mime.js
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/platform/contextkey/common/scanner.js
...
function hintDidYouMean(...meant) {
    switch (meant.length) {
        case 1:
            return nls_localize(1754, "Did you mean {0}?", meant[0]);
        case 2:
            return nls_localize(1755, "Did you mean {0} or {1}?", meant[0], meant[1]);
        case 3:
            return nls_localize(1756, "Did you mean {0}, {1} or {2}?", meant[0], meant[1], meant[2]);
        default:
            return undefined;
    }
}
const hintDidYouForgetToOpenOrCloseQuote = ( nls_localize(1757, "Did you forget to open or close the quote?"));
const hintDidYouForgetToEscapeSlash = ( nls_localize(
    1758,
...
class Scanner {
        ...
        while (!this._isAtEnd()) {
            ...
            switch (ch) {
                ...
                case charCode_CharCode.CloseParen:
                    ...
                    break;
                case charCode_CharCode.ExclamationMark:
                    if (this._match(charCode_CharCode.Equals)) {
                        const isTripleEq = this._match(charCode_CharCode.Equals);
                ...
                case charCode_CharCode.SingleQuote:
                    this._quotedString();
                    break;
                case charCode_CharCode.Slash:
                    this._regex();
                    break;
                case charCode_CharCode.Equals:
                    if (this._match(charCode_CharCode.Equals)) {
                        const isTripleEq = this._match(charCode_CharCode.Equals);
                        this._tokens.push({ type: TokenType.Eq, offset: this._start, isTripleEq });
                    }
                    else if (this._match(charCode_CharCode.Tilde)) {
                    ...
                    break;
                case charCode_CharCode.LessThan:
                    this._addToken(this._match(charCode_CharCode.Equals) ? TokenType.LtEq : TokenType.Lt);
                    break;
                case charCode_CharCode.GreaterThan:
                    this._addToken(this._match(charCode_CharCode.Equals) ? TokenType.GtEq : TokenType.Gt);
                    break;
                case charCode_CharCode.Ampersand:
                    if (this._match(charCode_CharCode.Ampersand)) {
                ...
                case charCode_CharCode.Pipe:
                    if (this._match(charCode_CharCode.Pipe)) {
                ...
                case charCode_CharCode.CarriageReturn:
                case charCode_CharCode.Tab:
                case charCode_CharCode.LineFeed:
                case charCode_CharCode.NoBreakSpace:
                    break;
                default:
                    this._string();
            }
    ...
    _peek() {
        return this._isAtEnd() ? charCode_CharCode.Null : this._input.charCodeAt(this._current);
    }
    _addToken(type) {
    ...
    _quotedString() {
        while (this._peek() !== charCode_CharCode.SingleQuote && !this._isAtEnd()) {
            this._advance();
        }
        if (this._isAtEnd()) {
    ...
    _regex() {
        ...
        while (true) {
            ...
            else if (ch === charCode_CharCode.Slash && !inCharacterClass) {
                p++;
                break;
            }
            else if (ch === charCode_CharCode.OpenSquareBracket) {
                inCharacterClass = true;
            }
            else if (ch === charCode_CharCode.Backslash) {
                inEscape = true;
            }
            else if (ch === charCode_CharCode.CloseSquareBracket) {
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/platform/contextkey/common/contextkey.js
...
CONSTANT_VALUES.set('isLinux', platform_isLinux);
CONSTANT_VALUES.set('isWindows', platform_isWindows);
...
const errorEmptyString = ( nls_localize(1734, "Empty context key expression"));
const hintEmptyString = ( nls_localize(
    1735,
    "Did you forget to write an expression? You can also put 'false' or 'true' to always evaluate to false or true, respectively."
));
const errorNoInAfterNot = ( nls_localize(1736, "'in' after 'not'."));
const errorClosingParenthesis = ( nls_localize(1737, "closing parenthesis ')'"));
const errorUnexpectedToken = ( nls_localize(1738, "Unexpected token"));
const hintUnexpectedToken = ( nls_localize(1739, "Did you forget to put && or || before the token?"));
const errorUnexpectedEOF = ( nls_localize(1740, "Unexpected end of expression"));
const hintUnexpectedEOF = ( nls_localize(1741, "Did you forget to put a context key?"));
class Parser {
                    ...
                    switch (expr.type) {
                        ...
                        case TokenType.Error: {
                            ...
                            for (let i = 0; i < expr.lexeme.length; i++) {
                                if (expr.lexeme.charCodeAt(i) === charCode_CharCode.OpenParen) {
                                    parenBalance++;
                                }
                                else if (expr.lexeme.charCodeAt(i) === charCode_CharCode.CloseParen) {
                                    parenBalance--;
                                }
                            }
                                ...
                                switch (followingToken.type) {
                                    ...
                                    case TokenType.QuotedStr:
                                        for (let i = 0; i < followingToken.lexeme.length; i++) {
                                            if (followingToken.lexeme.charCodeAt(i) === charCode_CharCode.OpenParen) {
                                                parenBalance++;
                                            }
                                            else if (expr.lexeme.charCodeAt(i) === charCode_CharCode.CloseParen) {
                                                parenBalance--;
    ...
    _errExpectedButGot(expected, got, additionalInfo) {
        const message = ( nls_localize(1742, "Expected: {0}\nReceived: '{1}'.", expected, Scanner.getLexeme(got)));
        ...
        if (parser.lexingErrors.length > 0) {
            return (parser.lexingErrors.map((se) => ({
                errorMessage: se.additionalInfo ?
                    ( localize(1743, "Unexpected token. Hint: {0}", se.additionalInfo)) :
                    ( localize(1744, "Unexpected token.")),
                offset: se.offset,
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/platform/instantiation/common/extensions.js



const _registry = (/* unused pure expression or super */ null && ([]));
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/platform/instantiation/common/instantiation.js
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/workbench/services/notebook/common/notebookDocumentService.js
...
const _radix = 7;
function notebookDocumentService_parse(cell) {
    if (cell.scheme !== network_Schemas.vscodeNotebookCell) {
        return undefined;
    }
...
function generate(notebook, handle) {
    ...
    const fragment = `${p}${s}s${encodeBase64(buffer_VSBuffer.fromString(notebook.scheme), true, true)}`;
    return notebook.with({ scheme: network_Schemas.vscodeNotebookCell, fragment });
}
function parseMetadataUri(metadata) {
    if (metadata.scheme !== network_Schemas.vscodeNotebookMetadata) {
        return undefined;
    }
...
function generateMetadataUri(notebook) {
    const fragment = `${encodeBase64(buffer_VSBuffer.fromString(notebook.scheme), true, true)}`;
    return notebook.with({ scheme: network_Schemas.vscodeNotebookMetadata, fragment });
}
function extractCellOutputDetails(uri) {
    if (uri.scheme !== network_Schemas.vscodeNotebookCellOutput) {
        return;
    }
    ...
    const parsedCell = notebookDocumentService_parse(uri.with({ scheme: network_Schemas.vscodeNotebookCell, query: null }));
    ...
    const notebookUri = parsedCell ? parsedCell.notebook : uri.with({
        scheme: params.get('notebookScheme') || network_Schemas.file,
        fragment: null,
        query: null,
    });
...
class NotebookDocumentWorkbenchService {
    ...
    getNotebook(uri) {
        if (uri.scheme === Schemas.vscodeNotebookCell) {
            const cellUri = notebookDocumentService_parse(uri);
            if (cellUri) {
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/workbench/contrib/notebook/common/notebookCommon.js
...
(function (NotebookMetadataUri) {
    NotebookMetadataUri.scheme = network_Schemas.vscodeNotebookMetadata;
...
var CellUri;
(function (CellUri) {
    CellUri.scheme = network_Schemas.vscodeNotebookCell;
    function generate$1(notebook, handle) {
        return generate(notebook, handle);
    }
    CellUri.generate = generate$1;
    function parse$1(cell) {
        return notebookDocumentService_parse(cell);
    }
    CellUri.parse = parse$1;
    function generateCellOutputUriWithId(notebook, outputId) {
        return notebook.with({
            scheme: network_Schemas.vscodeNotebookCellOutput,
            query: ( ( new URLSearchParams({
                openIn: 'editor',
                outputId: outputId ?? '',
                notebookScheme: notebook.scheme !== network_Schemas.file ? notebook.scheme : '',
            })).toString())
        });
    }
...
const notebookCommon_textDecoder = ( new TextDecoder());
...
function formatStreamText(buffer) {
    ...
    return VSBuffer.fromString(fixCarriageReturn(fixBackspace(notebookCommon_textDecoder.decode(buffer.buffer))));
}
...
function getWordAtText(column, wordDefinition, text, textOffset, config) {
    ...
    if (!config) {
        config = iterator_Iterable.first(_defaultConfig);
    }
...
;// ./node_modules/@codingame/monaco-vscode-notebook-service-override/vscode/src/vs/workbench/contrib/notebook/common/services/notebookCellMatching.js
...
;// ./node_modules/@codingame/monaco-vscode-api/vscode/src/vs/base/common/uuid.js
...
;// ./node_modules/@codingame/monaco-vscode-e4d0fd26-1b26-5583-b3f7-582e08d7b389-common/vscode/src/vs/workbench/contrib/notebook/common/notebookDiff.js
...
;// ./node_modules/@codingame/monaco-vscode-notebook-service-override/vscode/src/vs/workbench/contrib/notebook/common/services/notebookWebWorker.js
    ...
    _spliceNotebookCells(splices) {
        splices.reverse().forEach(splice => {
            ...
            const newCells = ( cellDtos.map(cell => {
                return ( new MirrorCell(cell.handle, ( uri_URI.parse(cell.url)), cell.source, cell.eol, cell.versionId, cell.language, cell.cellKind, cell.outputs, cell.metadata));
            }));
...
class NotebookWorker {
    ...
    $acceptNewModel(uri, metadata, transientDocumentMetadata, cells) {
        this._models[uri] = ( new MirrorNotebookDocument(( uri_URI.parse(uri)), ( cells.map(dto => ( new MirrorCell(dto.handle, ( uri_URI.parse(dto.url)), dto.source, dto.eol, dto.versionId, dto.language, dto.cellKind, dto.outputs, dto.metadata, dto.internalMetadata)))), metadata, transientDocumentMetadata));
    }
    ...
    async $computeDiff(originalUrl, modifiedUrl) {
        ...
        const cellMapping = computeDiff(originalModel, modifiedModel, { cellsDiff: { changes: originalDiff.changes, quitEarly: false }}).cellDiffInfo;
        ...
        const cellDiffInfo = computeDiff(originalModel, modifiedModel, { cellsDiff: { changes: cellsInsertedOrDeletedDiff.changes, quitEarly: false }}).cellDiffInfo;
    ...
    canComputeDiffWithCellInternalIds(original, modified) {
        const originalCellIndexIds = ( original.cells.map(
            (cell, index) => ({ index, id: (cell.internalMetadata?.internalId || '') })
        ));
        const modifiedCellIndexIds = ( modified.cells.map(
            (cell, index) => ({ index, id: (cell.internalMetadata?.internalId || '') })
        ));
    ...
    $canPromptRecommendation(modelUrl) {
        ...
        for (let i = 0; i < cells.length; i++) {
            ...
            const range = ( new range_Range(1, 1, maxLineCount, textBuffer.getLineLength(maxLineCount) + 1));
...
;// ./node_modules/@codingame/monaco-vscode-notebook-service-override/vscode/src/vs/workbench/contrib/notebook/common/services/notebookWebWorkerMain.js
